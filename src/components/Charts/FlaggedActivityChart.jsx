...
const FlaggedActivityChart = ({
  ...
  activityData = [
    ...
    },
    {
      ...
      trend: "+8%",
      icon: AlertCircle,
      route: "/dashboard/requested-bookings",
    },
  ],
}) => {
    ...
    switch (period) {
      case "Today":
        return baseData.map((item) => ({
          ...
          value: `৳${Math.floor(
            parseInt(item.value.replace(/[৳,]/g, "")) * 0.1
          ).toLocaleString()}`,
        }));
      case "Last 7 days":
        return baseData.map((item) => ({
          ...
          value: `৳${Math.floor(
            parseInt(item.value.replace(/[৳,]/g, "")) * 0.4
          ).toLocaleString()}`,
        }));
      case "Last 30 days":
        return baseData.map((item) => ({
          ...
          value: `৳${Math.floor(
            parseInt(item.value.replace(/[৳,]/g, "")) * 0.8
          ).toLocaleString()}`,
        }));
      case "All time":
      default:
  ...
  const handleExportReport = () => {
    ...
    link.href = url;
    link.download = `transaction-report-${activeTab
      .toLowerCase()
      .replace(" ", "-")}-${new Date().toISOString().split("T")[0]}.json`;
    link.click();
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {liveData.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={`${item.color} p-4 rounded-lg text-white relative overflow-hidden`}
            >
                ...
                >
                  <td className="py-4">
                    <div className="flex items-center">
                      <div
                        className={`w-3 h-3 ${item.color} rounded-full mr-3 flex items-center justify-center`}
                      >
      ...
      <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
        <div className="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400">
          <span>
            Showing {activeTab.toLowerCase()} activity •{" "}
            {getTotalTransactions()} total transactions
          </span>
          <button
